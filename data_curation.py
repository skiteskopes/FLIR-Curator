...
def image_viewer_function():
    ...
    window.iconbitmap('flam.ico')
    window.geometry("850x175")
    counter = 0
    '''initialize root window basics'''
    label1 = Label(window, text = 'dummy')
    def select_rgb():
        global label1
        label1.destroy()
    ...
    R4 = Radiobutton(window, text="Dual PRO 13mm 640, 32 deg HFOV", variable=radioselection, value=1, command=thirteen)
    R4.grid(row=4,column=1 )
    R5 = Radiobutton(window, text="Dual PRO 19mm 640, 32 deg HFOV", variable=radioselection, value=2, command=nineteen)
    R5.grid(row=5,column=1 )
    R6 = Radiobutton(window, text="Dual PRO 25mm 640, 32 deg HFOV", variable=radioselection, value=3, command=twentyfive)
    ...
    def open_images():
        ...
        def Next():
            ...
            if (counter+1) < 10:
                next_label = Label(canvas1, text = '0{0} of {1}'.format(counter+1, pair_amount), bg = 'navy', fg = 'white').place(x=width-21, y=height + 40)
            if (counter+1) >= 10:
        ...
        def Back():
            ...
            if (counter+1) < 10:
                back_label = Label(canvas1, text = '0{0} of {1}'.format(counter+1, pair_amount), bg = 'navy', fg = 'white').place(x=width-21, y=height + 40)
        ...
        def Delete():
            ...
            counter_list.append(delete_counter)
            directory = os.getcwd()
            trash = directory + r'\Trash'

            try:
                os.makedirs(trash)
            except FileExistsError:
                print ('Error: Creating directory. ' +  trash)
            ...
            grabrgb1 = path + str('/')+str(new_rgb[counter])
            grabir1 = path + str('/')+str(new_ir[counter])

            rgb_img1 = cv2.cvtColor(cv2.imread(grabrgb1), cv2.COLOR_BGR2RGB)
            rgb_scale_percent = 15
            rgb_width = int(rgb_img1.shape[1] * rgb_scale_percent / 100)
            rgb_height = int(rgb_img1.shape[0] * rgb_scale_percent / 100)
            dim1 = (rgb_width, rgb_height)
            ...
            irlabel2 =  Label(canvas1, text = '{0}'.format(new_ir[counter]), bg = 'navy', fg = 'white').place(x=width+563//2-50, y=height + 40)
        def Help():
            messagebox.showinfo('Help','Use the Left and Right arrow Keys to better navigate through the images. Delete transfers unwanted images into a newly created folder for future reference')
        ...
        def Jump():
            ...
            if value < 1:
                error_label.destroy()
                error_label = Label(canvas1, text="ERROR: value is out of bounds", fg = 'red', bg= 'white')
                error_label.place(x=width+ 150, y=height + 80)
            if value > pair_amount:
                error_label.destroy()
                error_label = Label(canvas1, text="ERROR: value is out of bounds", fg = 'red', bg= 'white')
                error_label.place(x=width+ 150, y=height + 80)
            if pair_amount >= value >= 1:
                error_label.destroy()
                reference_value = value-1
                if reference_value in counter_list:
                    error_label.destroy()
                    error_label = Label(canvas1, text="ERROR: value for images has been deleted", fg = 'red', bg= 'white')
                    error_label.place(x=width+ 150, y=height + 80)
                else:
                    error_label.destroy()
                    error_label = Label(canvas1, text="Jumped to Images", fg = 'green', bg= 'white')
                    error_label.place(x=width+ 150, y=height + 80)
                    grabrgb = path + str('/')+str(new_rgb[reference_value])
                    grabir = path + str('/')+str(new_ir[reference_value])
                    rgb_img1 = cv2.cvtColor(cv2.imread(grabrgb), cv2.COLOR_BGR2RGB)
                    rgb_scale_percent = 15
                    rgb_width = int(rgb_img1.shape[1] * rgb_scale_percent / 100)
                    rgb_height = int(rgb_img1.shape[0] * rgb_scale_percent / 100)
                    ...
                    canvas2 = canvas1.create_image(canvas2_w + width,canvas2_h,image=photo1)
                    if (value) < 10:
                        number_label = Label(canvas1, text = '0{0} of {1}'.format(value, pair_amount), bg = 'navy', fg = 'white')
                        number_label.place(x=width-21, y=height + 40)
                    if (value) >= 10:
                        number_label = Label(canvas1, text = '{0} of {1}'.format(value, pair_amount), bg = 'navy', fg = 'white')
                        number_label.place(x=width-21, y=height + 40)
                    counter = value -1
        def next_key(event):
            Next()
        def back_key(event):
            Back()
        window1.bind("<Right>",next_key)
        window1.bind("<Left>",back_key)
        ...
        number_label = Label(canvas1, text = '0{0} of {1}'.format(counter+1, pair_amount), bg = 'navy', fg = 'white')
        number_label.place(x=width-21, y=height + 40)
        next_button = Button(canvas1, text = "Next", command = Next, bg ='navy', fg='white').place(x=0,y=0)
        back_button = Button(canvas1, text="Back", command = Back, bg= 'navy', fg='white')
        back_button.place(x=35,y=0)
        delete_button = Button(canvas1, text="Delete", command = Delete, bg= 'navy', fg='white')
        delete_button.place(x=70,y=0)
        help_button = Button(canvas1, text="Help", command = Help, bg= 'navy', fg='white')
        help_button.place(x=112,y=0)
        entry_box = Entry(canvas1)
        entry_box.place(x=width+5, y=height + 80)
        entry_label = Label(canvas1, text="Jump to Image:", fg = 'white', bg= 'navy')
        entry_label.place(x=width-85, y=height + 80)
        entry_button = Button(canvas1, text="Jump", command = Jump, bg= 'white', fg='navy')
        entry_button.place(x=width+48, y=height + 110)
        rgbimage = PIL.Image.fromarray(rgb_img)
        photo = PIL.ImageTk.PhotoImage(rgbimage)
        canvas1_w = width/2
        canvas1_h = height/2 + 25
        ...
        photo1 = PIL.ImageTk.PhotoImage(irimage)
        canvas2 = canvas1.create_image(canvas2_w + width,canvas2_h,image=photo1)
        error_label = Label(canvas1, text="Dummy Label", fg = 'red', bg= 'white')
        'dummy error label during initialization to pre define label'
...
def frame_cropper_function():
    ...
    rgbbutton = Button(window, text = "Select", command =
    select_rgb,bg ='navy', fg='white')
    rgbbutton.grid(row =2, column =1)
    irbutton = Button(window, text = "Select", command =
    select_ir,bg ='navy', fg='white')
    irbutton.grid(row=3, column =1)
    ''' button widgets for selecting sample images'''
    ...
    R1 = Radiobutton(window, text="Dual PRO 13mm 640, 32 deg HFOV", variable=radioselection, value=1,
                      command=thirteen)
    R1.grid(row=4,column=1 )

    R2 = Radiobutton(window, text="Dual PRO 19mm 640, 32 deg HFOV", variable=radioselection, value=2,
                      command=nineteen)
    R2.grid(row=5,column=1 )

    R3 = Radiobutton(window, text="Dual PRO 25mm 640, 32 deg HFOV", variable=radioselection, value=3,
                      command=twentyfive)
    R3.grid(row=6,column=1 )
    '''radio button configuration'''
    R1.select()
    radiolabel = Label(window,text="Select focal length:").grid(row=4,column=0)
